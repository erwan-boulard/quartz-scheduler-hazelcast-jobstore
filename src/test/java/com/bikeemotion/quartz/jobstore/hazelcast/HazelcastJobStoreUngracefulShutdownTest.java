...
public class HazelcastJobStoreUngracefulShutdownTest extends AbstractTest {
    ...
    public void testOneOfTwoInstancesCrashing()
        ...
        final int numberOfTriggers = jobstore2.getNumberOfTriggers();
        assertEquals(numberOfTriggers,
                1,
                "Should find 1 trigger on node 2 after node 1 crashed when failing after 500ms");

        final int numberOfJobs = jobstore2.getNumberOfJobs();

        assertEquals(numberOfJobs,
                1,
                "Should find 1 job on node 2 after node 1 crashed when failing after 500ms");

        final List<OperableTrigger> triggers2 = jobstore2.acquireNextTriggers(firstFireTime + 150, 10, 0L);
        assertEquals(triggers2.size(), 1);
    }
}